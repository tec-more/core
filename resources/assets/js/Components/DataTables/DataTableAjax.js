class DataTableAjax {
    ...
    initEachItem(table, options) {
        let $table = $(table);
        let dataTableHelper = new Tuke.DataTable($table, options);
        ...
        dataTableHelper.getTableWrapper().on('confirmed.bs.confirmation', '.table-group-action-submit', function (e) {
            ...
            if (action.val() != "" && dataTableHelper.getSelectedRowsCount() > 0) {
                ...
                Tuke.showNotification('Please select an action', 'danger');
            } else if (dataTableHelper.getSelectedRowsCount() === 0) {
                Tuke.showNotification('No record selected', 'warning');
            }
        });
                ...
                complete: function (data) {
                    ...
                    if (typeof data.responseJSON !== 'undefined') {
                        if (data.responseJSON.error) {
                            Tuke.showNotification(data.responseJSON.messages, 'danger');
                        }
                        else {
                            Tuke.showNotification(data.responseJSON.messages, 'success');
                        }
                    }
                    else {
                        Tuke.showNotification('Some error occurred. View console log for more information', 'danger');
                    }
                    dataTableHelper.getDataTable().ajax.reload();
                }
            });
...
Tuke.DataTableAjax = DataTableAjax;